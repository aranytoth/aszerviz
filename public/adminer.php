...
script("$jh.onclick = function () { return confirm('".($Qe?js_escape($Qe):'Biztos benne?')."'); };","");}function
...
sprintf('%.3f mÃ¡sodperc',max(0,microtime(true)-$Hh));}function
relative_uri(){return
...
upload_error($l){$Ne=($l==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($l?'Nem tudom feltÃ¶lteni a fÃ¡jlt.'.($Ne?" ".sprintf('A maximÃ¡lis fÃ¡jlmÃ©ret %s B.',$Ne):""):'A fÃ¡jl nem lÃ©tezik.');}function
...
edit_form($Q,$n,$J,$Ni){global$b,$w,$si,$l;$Th=$b->tableName(table_status1($Q,true));page_header(($Ni?'Szerkeszt':'BeszÃºr'),$l,array("select"=>array($Q,$Th)),$Th);$b->editRowPrint($Q,$n,$J,$Ni);if($J===false)echo"<p class='error'>".'Nincs megjelenÃ­thetÅ‘ eredmÃ©ny.'."\n";echo'<form action="" method="post" enctype="multipart/form-data" id="form">
...
h($f->error);}function
check_sqlite_name($C){global$f;$Sc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Sc)\$~",$C)){$f->error=sprintf('HasznÃ¡lja a(z) %s kiterjesztÃ©st.',str_replace("|",", ",$Sc));return
false;}return
true;}function
create_database($j,$mb){global$f;if(file_exists($j)){$f->error='A fÃ¡jl lÃ©tezik.';return
...
as$j){if(!@unlink($j)){$f->error='A fÃ¡jl lÃ©tezik.';return
false;}}return
true;}function
rename_database($C,$mb){global$f;if(!check_sqlite_name($C))return
false;$f->__construct(":memory:");$f->error='A fÃ¡jl lÃ©tezik.';return@rename(DB,$C);}function
...
Min_DB;$Lb=$b->credentials();if($f->connect($Lb[0],$Lb[1],$Lb[2])){if(min_version(9,0,$f)){$f->query("SET application_name = 'Adminer'");if(min_version(9.2,0,$f)){$Lh['SzÃ¶veg'][]="json";$U["json"]=4294967295;if(min_version(9.4,0,$f)){$Lh['SzÃ¶veg'][]="jsonb";$U["jsonb"]=4294967295;}}}return$f;}return$f->error;}function
get_databases(){return
...
set_schema($eh,$g=null){global$f,$U,$Lh;if(!$g)$g=$f;$I=$g->query("SET search_path TO ".idf_escape($eh));foreach(types()as$T){if(!isset($U[$T])){$U[$T]=0;$Lh['FelhasznÃ¡lÃ³i tÃ­pus'][]=$T;}}return$I;}function
...
driver_config(){$U=array();$Lh=array();foreach(array('SzÃ¡m'=>array("smallint"=>5,"integer"=>10,"bigint"=>19,"boolean"=>1,"numeric"=>0,"real"=>7,"double precision"=>16,"money"=>20),'DÃ¡tum Ã©s idÅ‘'=>array("date"=>13,"time"=>17,"timestamp"=>20,"timestamptz"=>21,"interval"=>0),'SzÃ¶veg'=>array("character"=>0,"character varying"=>0,"text"=>0,"tsquery"=>0,"tsvector"=>0,"uuid"=>0,"xml"=>0),'BinÃ¡ris'=>array("bit"=>0,"bit varying"=>0,"bytea"=>0),'HÃ¡lÃ³zat'=>array("cidr"=>43,"inet"=>43,"macaddr"=>17,"txid_snapshot"=>0),'Geometria'=>array("box"=>0,"circle"=>0,"line"=>0,"lseg"=>0,"path"=>0,"point"=>0,"polygon"=>0),)as$x=>$X){$U+=$X;$Lh[$x]=array_keys($X);}return
...
driver_config(){$U=array();$Lh=array();foreach(array('SzÃ¡m'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'DÃ¡tum Ã©s idÅ‘'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'SzÃ¶veg'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'BinÃ¡ris'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$x=>$X){$U+=$X;$Lh[$x]=array_keys($X);}return
...
driver_config(){$U=array();$Lh=array();foreach(array('SzÃ¡m'=>array("tinyint"=>3,"smallint"=>5,"int"=>10,"bigint"=>20,"bit"=>1,"decimal"=>0,"real"=>12,"float"=>53,"smallmoney"=>10,"money"=>20),'DÃ¡tum Ã©s idÅ‘'=>array("date"=>10,"smalldatetime"=>19,"datetime"=>19,"datetime2"=>19,"time"=>8,"datetimeoffset"=>10),'SzÃ¶veg'=>array("char"=>8000,"varchar"=>8000,"text"=>2147483647,"nchar"=>4000,"nvarchar"=>4000,"ntext"=>1073741823),'BinÃ¡ris'=>array("binary"=>8000,"varbinary"=>8000,"image"=>2147483647),)as$x=>$X){$U+=$X;$Lh[$x]=array_keys($X);}return
...
last_id(){global$f;return$f->last_id;}function
driver_config(){$U=array();$Lh=array();foreach(array('SzÃ¡m'=>array("long"=>3,"integer"=>5,"short"=>8,"byte"=>10,"double"=>20,"float"=>66,"half_float"=>12,"scaled_float"=>21),'DÃ¡tum Ã©s idÅ‘'=>array("date"=>10),'SzÃ¶veg'=>array("string"=>65535,"text"=>65535),'BinÃ¡ris'=>array("binary"=>255),)as$x=>$X){$U+=$X;$Lh[$x]=array_keys($X);}return
...
loginForm(){global$mc;echo"<table cellspacing='0' class='layout'>\n",$this->loginFormField('driver','<tr><th>'.'AdatbÃ¡zis'.'<td>',html_select("auth[driver]",$mc,DRIVER,"loginDriver(this);")."\n"),$this->loginFormField('server','<tr><th>'.'Szerver'.'<td>','<input name="auth[server]" value="'.h(SERVER).'" title="hostname[:port]" placeholder="localhost" autocapitalize="off">'."\n"),$this->loginFormField('username','<tr><th>'.'FelhasznÃ¡lÃ³'.'<td>','<input name="auth[username]" id="username" value="'.h($_GET["username"]).'" autocomplete="username" autocapitalize="off">'.script("focus(qs('#username')); qs('#username').form['auth[driver]'].onchange();")),$this->loginFormField('password','<tr><th>'.'JelszÃ³'.'<td>','<input type="password" name="auth[password]" autocomplete="current-password">'."\n"),$this->loginFormField('db','<tr><th>'.'AdatbÃ¡zis'.'<td>','<input name="auth[db]" value="'.h($_GET["db"]).'" autocapitalize="off">'."\n"),"</table>\n","<p><input type='submit' value='".'BelÃ©pÃ©s'."'>\n",checkbox("auth[permanent]",1,$_COOKIE["adminer_permanent"],'EmlÃ©kezz rÃ¡m')."\n";}function
...
selectLinks($Sh,$N=""){global$w,$k;echo'<p class="links">';$wa=array("select"=>'Tartalom');if(support("table")||support("indexes"))$wa["table"]='StruktÃºra';if(support("table")){if(is_view($Sh))$wa["view"]='NÃ©zet mÃ³dosÃ­tÃ¡sa';else$wa["create"]='TÃ¡bla mÃ³dosÃ­tÃ¡sa';}if($N!==null)$wa["edit"]='Ãšj tÃ©tel';$C=$Sh["Name"];$_=[];foreach($wa
...
selectQuery($G,$Hh,$Vc=false){global$w,$k;if(!$Vc&&($gj=$k->warnings())){$s="warnings";$I=", <a href='#$s'>".'Warnings'."</a>".script("qsl('a').onclick = partial(toggle, '$s');","")."<div id='$s' class='hidden'>\n$gj</div>\n";}$_=[(support("sql")?"<a href='".h(ME)."sql=".urlencode($G)."'>".'Szerkeszt'."</a>":""),"<a href='#' class='copy-to-clipboard'>".'Copy to clipboard'."</a>",];return"<code class='jush-$w copy-to-clipboard'>".h(str_replace("\n"," ",$G))."</code> <span class='time'>(".format_time($Hh).")</span>".generate_linksbar($_);}function
sqlCommandQuery($G){return
shorten_utf8(trim($G),1000);}function
rowDescription($Q){return"";}function
rowDescriptions($K,$ld){return$K;}function
selectLink($X,$m){}function
selectVal($X,$z,$m,$Mf){$I=($X===null?"<i>NULL</i>":(preg_match("~char|binary|boolean~",$m["type"]??null)&&!preg_match("~var~",$m["type"]??null)?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$m["type"]??null)&&!is_utf8($X))$I="<i>".lang(array('%d bÃ¡jt','%d bÃ¡jt','%d bÃ¡jt'),strlen($Mf))."</i>";if(preg_match('~json~',$m["type"]??null))$I="<code class='jush-js'>$I</code>";return($z?"<a href='".h($z)."'".(is_url($z)?target_blank():"").">$I</a>":$I);}function
editVal($X,$m){return$X;}function
tableStructurePrint($n){echo"<div class='scrollable'>\n","<table cellspacing='0' class='nowrap'>\n","<thead><tr><th>".'Oszlop'."<td>".'TÃ­pus'.(support("comment")?"<td>".'MegjegyzÃ©s':"")."</thead>\n";foreach($n
as$m){echo"<tr".odd()."><th>".h($m["field"]),"<td><span title='".h($m["collation"])."'>".h($m["full_type"])."</span>",($m["null"]?" <i>NULL</i>":""),($m["auto_increment"]?" <i>".'Automatikus nÃ¶velÃ©s'."</i>":""),(isset($m["default"])?" <span title='".'Default value'."'>[<b>".h($m["default"])."</b>]</span>":""),(support("comment")?"<td>".h($m["comment"]):""),"\n";}echo"</table>\n","</div>\n";}function
...
selectColumnsPrint($L,$e){global$sd,$zd;print_fieldset("select",'KivÃ¡lasztÃ¡s',$L);$r=0;$L[""]=array();foreach($L
as$x=>$X){$X=$_GET["columns"][$x]??null;$d=select_input(" name='columns[$r][col]'",$e,$X["col"]??null,($x!==""?"selectFieldChange":"selectAddRow"));echo"<div>".($sd||$zd?"<select name='columns[$r][fun]'>".optionlist(array(-1=>"")+array_filter(array('FunkciÃ³k'=>$sd,'AggregÃ¡lÃ¡s'=>$zd)),$X["fun"]??null)."</select>".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'",1).script("qsl('select').onchange = function () { helpClose();".($x!==""?"":" qsl('select, input', this.parentNode).onchange();")." };","")."($d)":$d)." <input type='image' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.8.4")."' class='jsonly icon' title='",h('EltÃ¡volÃ­tÃ¡s'),"' alt='x'>".script('qsl(".icon").onclick = selectRemoveRow;',"")."</div>\n";$r++;}echo"</div></fieldset>\n";}function
selectSearchPrint($Z,$e,$v){print_fieldset("search",'KeresÃ©s',$Z);foreach($v
as$r=>$u){if($u["type"]=="FULLTEXT"){echo"<div>(<i>".implode("</i>, <i>",array_map('h',$u["columns"]))."</i>) AGAINST"," <input type='search' name='fulltext[$r]' value='".h($_GET["fulltext"][$r])."'>",script("qsl('input').oninput = selectFieldChange;",""),checkbox("boolean[$r]",1,isset($_GET["boolean"][$r]),"BOOL"),"</div>\n";}}$Za="this.parentNode.firstChild.onchange();";foreach(array_merge((array)$_GET["where"],array(array()))as$r=>$X){if(!$X||("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators))){echo"<div>".select_input(" name='where[$r][col]'",$e,$X["col"],($X?"selectFieldChange":"selectAddRow"),"(".'bÃ¡rhol'.")"),html_select("where[$r][op]",$this->operators,$X["op"],$Za),"<input type='search' name='where[$r][val]' value='".h($X["val"])."'>",script("mixin(qsl('input'), {oninput: function () { $Za }, onkeydown: selectSearchKeydown, onsearch: selectSearchSearch});",""),"<input type='image' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.8.4")."' class='jsonly icon' title='",h('EltÃ¡volÃ­tÃ¡s'),"' alt='x'>",script('qsl(".icon").onclick = selectRemoveRow;',""),"</div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($Ef,$e,$v){print_fieldset("sort",'Sorba rendezÃ©s',$Ef);$r=0;foreach((array)$_GET["order"]as$x=>$X){if($X!=""){echo"<div>".select_input(" name='order[$r]'",$e,$X,"selectFieldChange"),checkbox("desc[$r]",1,isset($_GET["desc"][$x]),'csÃ¶kkenÅ‘')," <input type='image' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.8.4")."' class='jsonly icon' title='",h('EltÃ¡volÃ­tÃ¡s'),"' alt='x'>",script('qsl(".icon").onclick = selectRemoveRow;',""),"</div>\n";$r++;}}echo"<div>".select_input(" name='order[$r]'",$e,"","selectAddRow"),checkbox("desc[$r]",1,false,'csÃ¶kkenÅ‘')," <input type='image' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.8.4")."' class='jsonly icon' title='",h('EltÃ¡volÃ­tÃ¡s'),"' alt='x'>",script('qsl(".icon").onclick = selectRemoveRow;',""),"</div>\n","</div></fieldset>\n";}function
selectLimitPrint($y){echo"<fieldset><legend>".'korlÃ¡t'."</legend><div>";echo"<input type='number' name='limit' class='size' value='".h($y)."'>",script("qsl('input').oninput = selectFieldChange;",""),"</div></fieldset>\n";}function
selectLengthPrint($hi){if($hi!==null){echo"<fieldset><legend>".'SzÃ¶veg hossz'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($hi)."'>","</div></fieldset>\n";}}function
selectActionPrint($v){echo"<fieldset><legend>".'MÅ±velet'."</legend><div>","<input type='submit' value='".'KivÃ¡lasztÃ¡s'."'>"," <span id='noindex' title='".'Full table scan'."'></span>","<script".nonce().">\n","var indexColumns = ";$e=array();foreach($v
...
as$fg=>$X){if(!$fg||preg_match("~$fg~",$m["type"]))$I.="/$X";}}if($x&&!preg_match('~set|blob|bytea|raw|file|bool~',$m["type"]))$I.="/SQL";}if($m["auto_increment"]&&!$Ni)$I='Automatikus nÃ¶velÃ©s';return
explode("/",$I);}function
editInput($Q,$m,$Ia,$Y){if($m["type"]=="enum"){$D=array();$ih=$Y;if(isset($_GET["select"])){$D[-1]='eredeti';if($ih===null)$ih=-1;}if($m["null"]){$D[""]="NULL";if($Y===null&&!isset($_GET["select"]))$ih="";}$D[0]='Ã¼res';preg_match_all("~'((?:[^']|'')*)'~",$m["length"],$Ie);foreach($Ie[1]as$r=>$X){$X=stripcslashes(str_replace("''","'",$X));$D[$r+1]=$X;if($Y===$X)$ih=$r+1;}return"<select$Ia>".optionlist($D,(string)$ih,1)."</select>";}return"";}function
...
dumpOutput(){$I=array('text'=>'megnyit','file'=>'ment');if(function_exists('gzencode'))$I['gz']='gzip';return$I;}function
dumpFormat(){return
...
homepage(){$_=[];if($_GET["ns"]==""&&support("database"))$_[]='<a href="'.h(ME).'database=">'.'AdatbÃ¡zis mÃ³dosÃ­tÃ¡sa'.'</a>';if(support("scheme"))$_[]="<a href='".h(ME)."scheme='>".($_GET["ns"]!=""?'SÃ©ma mÃ³dosÃ­tÃ¡sa':'SÃ©ma lÃ©trehozÃ¡sa')."</a>";if($_GET["ns"]!=="")$_[]='<a href="'.h(ME).'schema=">'.'AdatbÃ¡zis sÃ©ma'.'</a>';if(support("privileges"))$_[]="<a href='".h(ME)."privileges='>".'PrivilÃ©giumok'."</a>";echo
...
';}$this->databasesPrint($Xe);$_=[];if(DB==""||!$Xe){if(support("sql")){$_[]="<a href='".h(ME)."sql='".bold(isset($_GET["sql"])&&!isset($_GET["import"])).">".'SQL parancs'."</a>";$_[]="<a href='".h(ME)."import='".bold(isset($_GET["import"])).">".'ImportÃ¡lÃ¡s'."</a>";}if(support("dump"))$_[]="<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>";}echo
generate_linksbar($_);if($_GET["ns"]!==""&&!$Xe&&DB!=""){echo
generate_linksbar(['<a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'TÃ¡bla lÃ©trehozÃ¡sa'."</a>"]);if(!$S)echo"<p class='message'>".'Nincs tÃ¡bla.'."\n";else$this->tablesPrint($S);}}}function
databasesPrint($Xe){global$b,$f;$i=$this->databases();if(DB&&$i&&!in_array(DB,$i))array_unshift($i,DB);echo'<form action="">
',"<table id='dbs'><tr><td width=1>";hidden_fields_get();$Ub=script("mixin(qsl('select'), {onmousedown: dbMouseDown, onchange: dbChange});");echo"<label title='".'adatbÃ¡zis'."' for='menu_db'>".'DB'."</label>:</td><td>".($i?"<select name='db' id='menu_db'>".optionlist(array(""=>"")+$i,DB)."</select>$Ub":"<input name='db' id='menu_db' value='".h(DB)."' autocapitalize='off'>\n"),"</td></tr>";if(support("scheme")){if($Xe!="db"&&DB!=""&&$f->select_db(DB)){echo"<tr><td><label for='menu_ns'>".'SÃ©ma'.":</label></td>","<td><select name='ns' id='menu_ns'>".optionlist(array(""=>"")+$b->schemas(),$_GET["ns"])."</select>$Ub";if($_GET["ns"]!="")set_schema($_GET["ns"]);echo"</td></tr>";}}echo"<tr".($i?" class='hidden'":"")."><td colspan=2><input type='submit' value='".'HasznÃ¡l'."'></td></tr>\n","</table>";foreach(array("import","sql","schema","dump","privileges")as$X){if(isset($_GET[$X])){echo"<input type='hidden' name='$X' value=''>";break;}}}function
tablesPrint($S){echo"<ul id='tables'>".script("mixin(qs('#tables'), {onmouseover: menuOver, onmouseout: menuOut});");foreach($S
as$Q=>$O){$C=$this->tableName($O);if($C!=""){echo'<li><a href="'.h(ME).'select='.urlencode($Q).'"'.bold($_GET["select"]==$Q||$_GET["edit"]==$Q,"select")." title='".'Tartalom'."'>".'kivÃ¡lasztÃ¡s'."</a> ",(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($Q).'"'.bold(in_array($Q,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"],$_GET["select"])),(is_view($O)?"view":"structure"))." title='".'StruktÃºra'."'>$C</a>":"<span>$C</span>")."\n";}}echo"</ul>\n";}}$b=(function_exists('adminer_object')?adminer_object():new
...
driver_config(){$U=array();$Lh=array();foreach(array('SzÃ¡m'=>array("tinyint"=>3,"smallint"=>5,"mediumint"=>8,"int"=>10,"bigint"=>20,"decimal"=>66,"float"=>12,"double"=>21),'DÃ¡tum Ã©s idÅ‘'=>array("date"=>10,"datetime"=>19,"timestamp"=>19,"time"=>10,"year"=>4),'SzÃ¶veg'=>array("char"=>255,"varchar"=>65535,"tinytext"=>255,"text"=>65535,"mediumtext"=>16777215,"longtext"=>4294967295),'Lista'=>array("enum"=>65535,"set"=>64),'BinÃ¡ris'=>array("bit"=>20,"binary"=>255,"varbinary"=>65535,"tinyblob"=>255,"blob"=>65535,"mediumblob"=>16777215,"longblob"=>4294967295),'Geometria'=>array("geometry"=>0,"point"=>0,"linestring"=>0,"polygon"=>0,"multipoint"=>0,"multilinestring"=>0,"multipolygon"=>0,"geometrycollection"=>0),)as$x=>$X){$U+=$X;$Lh[$x]=array_keys($X);}return
...
<p class="logout">
<input type="submit" name="logout" value="KilÃ©pÃ©s" id="logout">
<input type="hidden" name="token" value="',$si,'">
</p>
</form>
';}echo'<div id="menu">
...
check_invalid_login(){global$b;$ee=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$de=($ee?$ee[$b->bruteForceKey()]:array());if($de===null)return;$gf=($de[1]>29?$de[0]-time():0);if($gf>0)auth_error(sprintf('Too many unsuccessful logins, try again in %d minute(s).',ceil($gf/60)));}$Ja=$_POST["auth"];if($Ja){session_regenerate_id();$aj=$Ja["driver"];$M=trim($Ja["server"]);$V=$Ja["username"];$F=(string)$Ja["password"];$j=$Ja["db"];set_password($aj,$M,$V,$F);$_SESSION["db"][$aj][$M][$V][$j]=true;if($Ja["permanent"]){$x=base64_encode($aj)."-".base64_encode($M)."-".base64_encode($V)."-".base64_encode($j);$tg=$b->permanentLogin(true);$hg[$x]="$x:".base64_encode($tg?encrypt_string($F,$tg):"");cookie("adminer_permanent",implode(" ",$hg));}if(count($_POST)==1||DRIVER!=$aj||SERVER!=$M||$_GET["username"]!==$V||DB!=$j)redirect(auth_url($aj,$M,$V,$j));}elseif($_POST["logout"]&&(!$Dd||verify_token())){foreach(array("pwds","db","dbs","queries")as$x)set_session($x,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Sikeres kilÃ©pÃ©s.');}elseif($hg&&!$_SESSION["pwds"]){session_regenerate_id();$tg=$b->permanentLogin();foreach($hg
...
auth_error($l){global$b,$Dd;$rh=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$rh]||$_GET[$rh])&&!$Dd)$l='Munkamenet lejÃ¡rt, jelentkezz be Ãºjra.';else{restart_session();add_invalid_login();$F=get_password();if($F!==null){if($F===false)$l.=($l?'<br>':'').sprintf('Master password expired. <a href="https://www.adminer.org/en/extension/"%s>Implement</a> %s method to make it permanent.',target_blank(),'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$rh]&&$_GET[$rh]&&ini_bool("session.use_only_cookies"))$l='A munkameneteknek (session) engedÃ©lyezve kell lennie.';$Uf=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$Uf["lifetime"]);page_header('BelÃ©pÃ©s',$l,null);echo"<form action='' method='post'>\n","<div>";if(hidden_fields($_POST,array("auth")))echo"<p class='message'>".'The action will be performed after successful login with the same credentials.'."\n";echo"</div>\n";$b->loginForm();echo"</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])&&!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Nincs kiterjesztÃ©s',sprintf('Nincs egy elÃ©rhetÅ‘ tÃ¡mogatott PHP kiterjesztÃ©s (%s) sem.',implode(", ",$ng)),false);page_footer("auth");exit;}stop_session(true);if(isset($_GET["username"])&&is_string(get_password())){validate_server_input();check_invalid_login();$f=connect();$k=new
Min_Driver($f);}$Ce=null;if(!is_object($f)||($Ce=$b->login($_GET["username"],get_password()))!==true){$l=(is_string($f)?h($f):(is_string($Ce)?$Ce:'Invalid server or credentials.'));auth_error($l.(preg_match('~^ | $~',get_password())?'<br>'.'There is a space in the input password which might be the cause.':''));}if($_POST["logout"]&&$Dd&&!verify_token()){page_header('KilÃ©pÃ©s','Ã‰rvÃ©nytelen CSRF azonosÃ­tÃ³. KÃ¼ldd Ãºjra az Å±rlapot.');page_footer("db");exit;}if($Ja&&$_POST["token"])$_POST["token"]=$si;$l='';if($_POST){if(!verify_token()){$Yd="max_input_vars";$Oe=ini_get($Yd);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$x){$X=ini_get($x);if($X&&(!$Oe||$X<$Oe)){$Yd=$x;$Oe=$X;}}}$l=(!$_POST["token"]&&$Oe?sprintf('A maximÃ¡lis mezÅ‘szÃ¡mot elÃ©rted. NÃ¶veld meg ezeket: %s.',"'$Yd'"):'Ã‰rvÃ©nytelen CSRF azonosÃ­tÃ³. KÃ¼ldd Ãºjra az Å±rlapot.'.' '.'If you did not send this request from Adminer then close this page.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$l=sprintf('TÃºl sok a POST adat! CsÃ¶kkentsd az adat mÃ©retÃ©t, vagy nÃ¶veld a %s beÃ¡llÃ­tÃ¡st.',"'post_max_size'");if(isset($_GET["sql"]))$l.=' '.'You can upload a big SQL file via FTP and import it from server.';}function
...
as$x=>$X){$z="";if(isset($_[$x])&&!$e[$_[$x]]){if($Hf&&$w=="sql"){$Q=$J[array_search("table=",$_)];$z=ME.$_[$x].urlencode($Hf[$Q]!=""?$Hf[$Q]:$Q);}else{$z=ME."edit=".urlencode($_[$x]);foreach($v[$_[$x]]as$kb=>$ke)$z.="&where".urlencode("[".bracket_escape($kb)."]")."=".urlencode($J[$ke]);}}elseif(is_url($X))$z=$X;if($X===null)$X="<i>NULL</i>";elseif($Ta[$x]&&!is_utf8($X))$X="<i>".lang(array('%d bÃ¡jt','%d bÃ¡jt','%d bÃ¡jt'),strlen($X))."</i>";else{$X=h($X);if($U[$x]==254)$X="<code>$X</code>";}if($z)$X="<a href='".h($z)."'".(is_url($z)?target_blank():'').">$X</a>";echo"<td>$X";}}echo($r?"</table>\n</div>":"<p class='message'>".'Nincs megjelenÃ­thetÅ‘ eredmÃ©ny.')."\n";return$I;}function
...
edit_type($x,$m,$nb,$md=array(),$Tc=array()){global$Lh,$U,$Mi,$vf;$T=$m["type"];echo'<td><select name="',h($x),'[type]" class="type" aria-labelledby="label-type">';if($T&&!isset($U[$T])&&!isset($md[$T])&&!in_array($T,$Tc))$Tc[]=$T;if($md)$Lh['Idegen kulcs']=$md;echo
optionlist(array_merge($Tc,$Lh),$T),'</select><td><input name="',h($x),'[length]" value="',h($m["length"]),'" size="3"',(!$m["length"]&&preg_match('~var(char|binary)$~',$T)?" class='required'":"");echo' aria-labelledby="label-length"><td class="options">',"<select name='".h($x)."[collation]'".(preg_match('~(char|text|enum|set)$~',$T)?"":" class='hidden'").'><option value="">('.'egybevetÃ©s'.')'.optionlist($nb,$m["collation"]).'</select>',($Mi?"<select name='".h($x)."[unsigned]'".(!$T||preg_match(number_type(),$T)?"":" class='hidden'").'><option>'.optionlist($Mi,$m["unsigned"]).'</select>':''),(isset($m['on_update'])?"<select name='".h($x)."[on_update]'".(preg_match('~timestamp|datetime~',$T)?"":" class='hidden'").'>'.optionlist(array(""=>"(".'frissÃ­tÃ©skor'.")","CURRENT_TIMESTAMP"),(preg_match('~^CURRENT_TIMESTAMP~i',$m["on_update"])?"CURRENT_TIMESTAMP":$m["on_update"])).'</select>':''),($md?"<select name='".h($x)."[on_delete]'".(preg_match("~`~",$T)?"":" class='hidden'")."><option value=''>(".'tÃ¶rlÃ©skor'.")".optionlist(explode("|",$vf),$m["on_delete"])."</select> ":" ");}function
...
';if($T=="PROCEDURE"){echo'<td>';}echo'<th id="label-name">',($T=="TABLE"?'Oszlop neve':'ParamÃ©ter neve'),'<td id="label-type">TÃ­pus<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;"></textarea>',script("qs('#enum-edit').onblur = editingLengthBlur;"),'<td id="label-length">Hossz
<td>','OpciÃ³k';if($T=="TABLE"){echo'<td id="label-null">NULL
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="Automatikus nÃ¶velÃ©s">AI</acronym>',doc_link(array('sql'=>"example-auto-increment.html",'mariadb'=>"auto_increment/",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td id="label-default"',$bc,'>Default value
',(support("comment")?"<td id='label-comment'$ub>".'MegjegyzÃ©s':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($n))."]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.8.4")."' alt='+' title='".'KÃ¶vetkezÅ‘ hozzÃ¡adÃ¡sa'."'>".script("row_count = ".count($n).";"),'</thead>
<tbody>
...
connect_error(){global$b,$f,$si,$l,$mc;if(DB!=""){header("HTTP/1.1 404 Not Found");page_header('AdatbÃ¡zis'.": ".h(DB),'Ã‰rvÃ©nytelen adatbÃ¡zis.',true);}else{if($_POST["db"]&&!$l)queries_redirect(substr(ME,0,-1),'AdatbÃ¡zis eldobva.',drop_databases($_POST["db"]));page_header('AdatbÃ¡zis kivÃ¡lasztÃ¡sa',$l,false);$wa=['database'=>'AdatbÃ¡zis lÃ©trehozÃ¡sa','privileges'=>'PrivilÃ©giumok','processlist'=>'Folyamatok','variables'=>'VÃ¡ltozÃ³k','status'=>'Ãllapot',];$_=[];foreach($wa
as$x=>$X){if(support($x))$_[]="<a href='".h(ME)."$x='>$X</a>";}echo
generate_linksbar($_),"<p>".sprintf('%s verziÃ³: %s, PHP: %s',$mc[DRIVER],"<b>".h($f->server_info)."</b>","<b>$f->extension</b>")."\n","<p>".sprintf('BelÃ©pve: %s',"<b>".h(logged_user())."</b>")."\n";$i=$b->databases();if($i){$fh=support("scheme");$nb=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable'>\n",script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});"),"<thead><tr>".(support("database")?"<td>":"")."<th>".'AdatbÃ¡zis'." - <a href='".h(ME)."refresh=1'>".'FrissÃ­tÃ©s'."</a>"."<td>".'EgybevetÃ©s'."<td>".'TÃ¡blÃ¡k'."<td>".'Size'." - <a href='".h(ME)."dbsize=1'>".'Compute'."</a>".script("qsl('a').onclick = partial(ajaxSetHtml, '".js_escape(ME)."script=connect');","")."</thead>\n";$i=($_GET["dbsize"]?count_tables($i):array_flip($i));foreach($i
as$j=>$S){$Xg=h(ME)."db=".urlencode($j);$s=h("Db-".$j);echo"<tr".odd().">".(support("database")?"<td>".checkbox("db[]",$j,in_array($j,(array)$_POST["db"]),"","","",$s):""),"<th><a href='$Xg' id='$s'>".h($j)."</a>";$mb=h(db_collation($j,$nb));echo"<td>".(support("database")?"<a href='$Xg".($fh?"&amp;ns=":"")."&amp;database=' title='".'AdatbÃ¡zis mÃ³dosÃ­tÃ¡sa'."'>$mb</a>":$mb),"<td align='right'><a href='$Xg&amp;schema=' id='tables-".h($j)."' title='".'AdatbÃ¡zis sÃ©ma'."'>".($_GET["dbsize"]?$S:"?")."</a>","<td align='right' id='size-".h($j)."'>".($_GET["dbsize"]?db_size($j):"?"),"\n";}echo"</table>\n",(support("database")?"<div class='footer'><div>\n"."<fieldset><legend>".'Selected'." <span id='selected'></span></legend><div>\n"."<input type='hidden' name='all' value=''>".script("qsl('input').onclick = function () { selectCount('selected', formChecked(this, /^db/)); };")."<input type='submit' name='drop' value='".'Eldob'."'>".confirm()."\n"."</div></fieldset>\n"."</div></div>\n":""),"<input type='hidden' name='token' value='$si'>\n","</form>\n",script("tableCheck();");}}page_footer("db");}if(isset($_GET["status"]))$_GET["variables"]=$_GET["status"];if(isset($_GET["import"]))$_GET["sql"]=$_GET["import"];if(!(DB!=""?$f->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["variables"])||$_GET["script"]=="connect"||$_GET["script"]=="kill")){if(DB!=""||$_GET["refresh"]){restart_session();set_session("dbs",null);}connect_error();exit;}if(support("scheme")){if(DB!=""&&$_GET["ns"]!==""){if(!isset($_GET["ns"]))redirect(preg_replace('~ns=[^&]*&~','',ME)."ns=".get_schema());if(!set_schema($_GET["ns"])){header("HTTP/1.1 404 Not Found");page_header('SÃ©ma'.": ".h($_GET["ns"]),'Ã‰rvÃ©nytelen sÃ©ma.',true);page_footer("ns");exit;}}}$vf="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";class
TmpFile{var$handler;var$size;function
__construct(){$this->handler=tmpfile();}function
write($Db){$this->size+=strlen($Db);fwrite($this->handler,$Db);}function
send(){fseek($this->handler,0);fpassthru($this->handler);fclose($this->handler);}}$Dc="'(?:''|[^'\\\\]|\\\\.)*'";$Zd="IN|OUT|INOUT";if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["callf"]))$_GET["call"]=$_GET["callf"];if(isset($_GET["function"]))$_GET["procedure"]=$_GET["function"];if(isset($_GET["download"])){$a=$_GET["download"];$n=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));$L=array(idf_escape($_GET["field"]));$H=$k->select($a,$L,array(where($_GET,$n)),$L);$J=($H?$H->fetch_row():array());echo$k->value($J[0],$n[$_GET["field"]]);exit;}elseif(isset($_GET["table"])){$a=$_GET["table"];$n=fields($a);if(!$n)$l=error();$R=table_status1($a,true);$C=$b->tableName($R);page_header(($n&&is_view($R)?$R['Engine']=='materialized view'?'Materialized view':'NÃ©zet':'TÃ¡bla').": ".($C!=""?$C:h($a)),$l);$b->selectLinks($R);$tb=$R["Comment"];if($tb!="")echo"<p class='nowrap'>".'MegjegyzÃ©s'.": ".h($tb)."\n";if($n)$b->tableStructurePrint($n);if(!is_view($R)){if(support("indexes")){echo"<h3 id='indexes'>".'Indexek'."</h3>\n";$v=indexes($a);if($v)$b->tableIndexesPrint($v);echo'<p class="links"><a href="'.h(ME).'indexes='.urlencode($a).'">'.'Index mÃ³dosÃ­tÃ¡sa'."</a>\n";}if(fk_support($R)){echo"<h3 id='foreign-keys'>".'Idegen kulcs'."</h3>\n";$md=foreign_keys($a);if($md){echo"<table cellspacing='0'>\n","<thead><tr><th>".'ForrÃ¡s'."<td>".'CÃ©l'."<td>".'tÃ¶rlÃ©skor'."<td>".'frissÃ­tÃ©skor'."<td></thead>\n";foreach($md
as$C=>$p){echo"<tr title='".h($C)."'>","<th><i>".implode("</i>, <i>",array_map('h',$p["source"]))."</i>","<td><a href='".h($p["db"]!=""?preg_replace('~db=[^&]*~',"db=".urlencode($p["db"]),ME):($p["ns"]!=""?preg_replace('~ns=[^&]*~',"ns=".urlencode($p["ns"]),ME):ME))."table=".urlencode($p["table"])."'>".($p["db"]!=""?"<b>".h($p["db"])."</b>.":"").($p["ns"]!=""?"<b>".h($p["ns"])."</b>.":"").h($p["table"])."</a>","(<i>".implode("</i>, <i>",array_map('h',$p["target"]))."</i>)","<td>".h($p["on_delete"])."\n","<td>".h($p["on_update"])."\n",'<td><a href="'.h(ME.'foreign='.urlencode($a).'&name='.urlencode($C)).'">'.'MÃ³dosÃ­tÃ¡s'.'</a>';}echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'foreign='.urlencode($a).'">'.'Idegen kulcs hozzadÃ¡sa'."</a>\n";}}if(support(is_view($R)?"view_trigger":"trigger")){echo"<h3 id='triggers'>".'Trigger'."</h3>\n";$Di=triggers($a);if($Di){echo"<table cellspacing='0'>\n";foreach($Di
as$x=>$X)echo"<tr valign='top'><td>".h($X[0])."<td>".h($X[1])."<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".'MÃ³dosÃ­tÃ¡s'."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Trigger hozzÃ¡adÃ¡sa'."</a>\n";}}elseif(isset($_GET["schema"])){page_header('AdatbÃ¡zis sÃ©ma',"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Vh=array();$Wh=array();$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE["adminer_schema-".str_replace(".","_",DB)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Ie,PREG_SET_ORDER);foreach($Ie
...
<p class="links"><a href="',h(ME."schema=".urlencode($ea)),'" id="schema-link">HivatkozÃ¡s</a>
...
';$Vb=array('','USE','DROP+CREATE','CREATE');$Xh=array('','DROP+CREATE','CREATE');$Rb=array('','TRUNCATE+INSERT','INSERT');if($w=="sql")$Rb[]='INSERT+UPDATE';parse_str($_COOKIE["adminer_export"],$J);if(!$J)$J=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");if(!isset($J["events"])){$J["routines"]=$J["events"]=($_GET["dump"]=="");$J["triggers"]=$J["table_style"];}echo"<tr><th>".'Kimenet'."<td>".html_select("output",$b->dumpOutput(),$J["output"],0)."\n";echo"<tr><th>".'FormÃ¡tum'."<td>".html_select("format",$b->dumpFormat(),$J["format"],0)."\n";echo($w=="sqlite"?"":"<tr><th>".'AdatbÃ¡zis'."<td>".html_select('db_style',$Vb,$J["db_style"]).(support("routine")?checkbox("routines",1,$J["routines"],'Rutinok'):"").(support("event")?checkbox("events",1,$J["events"],'EsemÃ©ny'):"")),"<tr><th>".'TÃ¡blÃ¡k'."<td>".html_select('table_style',$Xh,$J["table_style"]).checkbox("auto_increment",1,$J["auto_increment"],'Automatikus nÃ¶velÃ©s').(support("trigger")?checkbox("triggers",1,$J["triggers"],'Trigger'):""),"<tr><th>".'Adat'."<td>".html_select('data_style',$Rb,$J["data_style"]),'</table>
<p><input type="submit" value="Export">
<input type="hidden" name="token" value="',$si,'">

<table cellspacing="0">
',script("qsl('table').onclick = dumpClick;");$pg=array();if(DB!=""){$db=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$db>".'TÃ¡blÃ¡k'."</label>".script("qs('#check-tables').onclick = partial(formCheck, /^tables\\[/);",""),"<th style='text-align: right;'><label class='block'>".'Adat'."<input type='checkbox' id='check-data'$db></label>".script("qs('#check-data').onclick = partial(formCheck, /^data\\[/);",""),"</thead>\n";$dj="";$Yh=tables_list();foreach($Yh
...
script("ajaxSetHtml('".js_escape(ME)."script=db');");}else{echo"<thead><tr><th style='text-align: left;'>","<label class='block'><input type='checkbox' id='check-databases'".($a==""?" checked":"").">".'AdatbÃ¡zis'."</label>",script("qs('#check-databases').onclick = partial(formCheck, /^databases\\[/);",""),"</thead>\n";$i=$b->databases();if($i){foreach($i
as$j){if(!information_schema($j)){$og=preg_replace('~_.*~','',$j);echo"<tr><td>".checkbox("databases[]",$j,$a==""||$a=="$og%",$j,"","block")."\n";$pg[$og]++;}}}else
echo"<tr><td><textarea name='databases' rows='10' cols='20'></textarea>";}echo'</table>
</form>
';$dd=true;foreach($pg
as$x=>$X){if($x!=""&&$X>1){echo($dd?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$x%")."'>".h($x)."</a>";$dd=false;}}}elseif(isset($_GET["privileges"])){page_header('PrivilÃ©giumok');echo'<p class="links"><a href="'.h(ME).'user=">'.'FelhasznÃ¡lÃ³ hozzÃ¡adÃ¡sa'."</a>";$H=$f->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$ud=$H;if(!$H)$H=$f->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($ud?"":"<input type='hidden' name='grant' value=''>\n"),"<table cellspacing='0'>\n","<thead><tr><th>".'FelhasznÃ¡lÃ³'."<th>".'Szerver'."<th></thead>\n";while($J=$H->fetch_assoc())echo'<tr'.odd().'><td>'.h($J["User"])."<td>".h($J["Host"]).'<td><a href="'.h(ME.'user='.urlencode($J["User"]).'&host='.urlencode($J["Host"])).'">'.'Szerkeszt'."</a>\n";if(!$ud||DB!="")echo"<tr".odd()."><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".'Szerkeszt'."'>\n";echo"</table>\n","</form>\n";}elseif(isset($_GET["sql"])){if(!$l&&$_POST["export"]){dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);exit;}restart_session();$Hd=&get_session("queries");$Gd=&$Hd[DB];if(!$l&&$_POST["clear"]){$Gd=array();redirect(remove_from_uri("history"));}page_header((isset($_GET["import"])?'ImportÃ¡lÃ¡s':'SQL parancs'),$l);if(!$l&&$_POST){$qd=false;if(!isset($_GET["import"]))$G=$_POST["query"];elseif($_POST["webfile"]){$Dh=$b->importServerPath();$qd=@fopen((file_exists($Dh)?$Dh:"compress.zlib://$Dh.gz"),"rb");$G=($qd?fread($qd,1e6):false);}else$G=get_file("sql_file",true);if(is_string($G)){if(function_exists('memory_get_usage'))@ini_set("memory_limit",max(ini_bytes("memory_limit"),2*strlen($G)+memory_get_usage()+8e6));if($G!=""&&strlen($G)<1e6){$zg=$G.(preg_match("~;[ \t\r\n]*\$~",$G)?"":";");if(!$Gd||reset(end($Gd))!=$zg){restart_session();$Gd[]=array($zg,time());set_session("queries",$Hd);stop_session();}}$Ah="(?:\\s|/\\*[\s\S]*?\\*/|(?:#|-- )[^\n]*\n?|--\r?\n)";$ec=";";$nf=0;$Ac=true;$g=connect();if(is_object($g)&&DB!=""){$g->select_db(DB);if($_GET["ns"]!="")set_schema($_GET["ns"],$g);}$sb=0;$Fc=array();$Vf='[\'"'.($w=="sql"?'`#':($w=="sqlite"?'`[':($w=="mssql"?'[':''))).']|/\*|-- |$'.($w=="pgsql"?'|\$[^$]*\$':'');$ui=microtime(true);parse_str($_COOKIE["adminer_export"],$ya);$sc=$b->dumpFormat();unset($sc["sql"]);while($G!=""){if(!$nf&&preg_match("~^$Ah*+DELIMITER\\s+(\\S+)~i",$G,$B)){$ec=$B[1];$G=substr($G,strlen($B[0]));}else{preg_match('('.preg_quote($ec)."\\s*|$Vf)",$G,$B,PREG_OFFSET_CAPTURE,$nf);list($od,$kg)=$B[0];if(!$od&&$qd&&!feof($qd))$G.=fread($qd,1e5);else{if(!$od&&rtrim($G)=="")break;$nf=$kg+strlen($od);if($od&&rtrim($od)!=$ec){while(preg_match('('.($od=='/*'?'\*/':($od=='['?']':(preg_match('~^-- |^#~',$od)?"\n":preg_quote($od)."|\\\\."))).'|$)s',$G,$B,PREG_OFFSET_CAPTURE,$nf)){$ch=$B[0][0];if(!$ch&&$qd&&!feof($qd))$G.=fread($qd,1e5);else{$nf=$B[0][1]+strlen($ch);if($ch[0]!="\\")break;}}}else{$Ac=false;$zg=substr($G,0,$kg);$sb++;$sg="<pre id='sql-$sb'><code class='jush-$w copy-to-clipboard'>".$b->sqlCommandQuery($zg)."</code></pre>\n";$sg.=generate_linksbar(["<a href='#' class='copy-to-clipboard'>".'Copy to clipboard'."</a>"]);if($w=="sqlite"&&preg_match("~^$Ah*+ATTACH\\b~i",$zg,$B)){echo$sg,"<p class='error'>".'ATTACH queries are not supported.'."\n";$Fc[]=" <a href='#sql-$sb'>$sb</a>";if($_POST["error_stops"])break;}else{if(!$_POST["only_errors"]){echo$sg;ob_flush();flush();}$Hh=microtime(true);if($f->multi_query($zg)&&is_object($g)&&preg_match("~^$Ah*+USE\\b~i",$zg))$g->query($zg);do{$H=$f->store_result();if($f->error){echo($_POST["only_errors"]?$sg:""),"<p class='error'>".'Hiba a lekÃ©rdezÃ©sben'.($f->errno?" ($f->errno)":"").": ".error()."\n";$Fc[]=" <a href='#sql-$sb'>$sb</a>";if($_POST["error_stops"])break
2;}else{$ii=" <span class='time'>(".format_time($Hh).")</span>".(strlen($zg)<1000?" <a href='".h(ME)."sql=".urlencode(trim($zg))."'>".'Szerkeszt'."</a>":"");$_a=$f->affected_rows;$gj=($_POST["only_errors"]?"":$k->warnings());$hj="warnings-$sb";if($gj)$ii.=", <a href='#$hj'>".'Warnings'."</a>".script("qsl('a').onclick = partial(toggle, '$hj');","");$Nc=null;$Oc="explain-$sb";if(is_object($H)){$y=$_POST["limit"];$Hf=select($H,$g,array(),$y);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n";$jf=$H->num_rows;echo"<p>".($jf?($y&&$jf>$y?sprintf('%d / ',$y):"").lang(array('%d sor','%d sor','%d sor'),$jf):""),$ii;if($g&&preg_match("~^($Ah|\\()*+SELECT\\b~i",$zg)&&($Nc=explain($g,$zg)))echo", <a href='#$Oc'>Explain</a>".script("qsl('a').onclick = partial(toggle, '$Oc');","");$s="export-$sb";echo", <a href='#$s'>".'Export'."</a>".script("qsl('a').onclick = partial(toggle, '$s');","")."<span id='$s' class='hidden'>: ".html_select("output",$b->dumpOutput(),$ya["output"])." ".html_select("format",$sc,$ya["format"])."<input type='hidden' name='query' value='".h($zg)."'>"." <input type='submit' name='export' value='".'Export'."'><input type='hidden' name='token' value='$si'></span>\n"."</form>\n";}}else{if(preg_match("~^$Ah*+(CREATE|DROP|ALTER)$Ah++(DATABASE|SCHEMA)\\b~i",$zg)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($f->info)."'>".lang(array('LekÃ©rdezÃ©s sikeresen vÃ©grehajtva, %d sor Ã©rintett.','LekÃ©rdezÃ©s sikeresen vÃ©grehajtva, %d sor Ã©rintett.','LekÃ©rdezÃ©s sikeresen vÃ©grehajtva, %d sor Ã©rintett.'),$_a)."$ii\n";}echo($gj?"<div id='$hj' class='hidden'>\n$gj</div>\n":"");if($Nc){echo"<div id='$Oc' class='hidden'>\n";select($Nc,$g,$Hf);echo"</div>\n";}}$Hh=microtime(true);}while($f->next_result());}$G=substr($G,$nf);$nf=0;}}}}if($Ac)echo"<p class='message'>".'Nincs vÃ©grehajthatÃ³ parancs.'."\n";elseif($_POST["only_errors"]){echo"<p class='message'>".sprintf('%d sikeres lekÃ©rdezÃ©s.',$sb-count($Fc))," <span class='time'>(".format_time($ui).")</span>\n";}elseif($Fc&&$sb>1)echo"<p class='error'>".'Hiba a lekÃ©rdezÃ©sben'.": ".implode("",$Fc)."\n";}else
echo"<p class='error'>".upload_error($G)."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';$Lc="<input type='submit' value='".'VÃ©grehajt'."' title='Ctrl+Enter'>";if(!isset($_GET["import"])){$zg=$_GET["sql"];if($_POST)$zg=$_POST["query"];elseif($_GET["history"]=="all")$zg=$Gd;elseif($_GET["history"]!="")$zg=$Gd[$_GET["history"]][0];echo"<p>";textarea("query",$zg,20);echo
script(($_POST?"":"qs('textarea').focus();\n")."qs('#form').onsubmit = partial(sqlSubmit, qs('#form'), '".js_escape(remove_from_uri("sql|limit|error_stops|only_errors|history"))."');"),"<p>$Lc\n",'Limit rows'.": <input type='number' name='limit' class='size' value='".h($_POST?$_POST["limit"]:$_GET["limit"])."'>\n";}else{echo"<fieldset><legend>".'FÃ¡jl feltÃ¶ltÃ©se'."</legend><div>";$_d=(extension_loaded("zlib")?"[.gz]":"");echo(ini_bool("file_uploads")?"SQL$_d (&lt; ".ini_get("upload_max_filesize")."B): <input type='file' name='sql_file[]' multiple>\n$Lc":'A fÃ¡jl feltÃ¶ltÃ©s le van tiltva.'),"</div></fieldset>\n";$Od=$b->importServerPath();if($Od){echo"<fieldset><legend>".'SzerverrÅ‘l'."</legend><div>",sprintf('Webszerver fÃ¡jl %s',"<code>".h($Od)."$_d</code>"),' <input type="submit" name="webfile" value="'.'FÃ¡jl futtatÃ¡sa'.'">',"</div></fieldset>\n";}echo"<p>";}echo
checkbox("error_stops",1,($_POST?$_POST["error_stops"]:isset($_GET["import"])||$_GET["error_stops"]),'Hiba esetÃ©n megÃ¡ll')."\n",checkbox("only_errors",1,($_POST?$_POST["only_errors"]:isset($_GET["import"])||$_GET["only_errors"]),'Csak a hibÃ¡k mutatÃ¡sa')."\n","<input type='hidden' name='token' value='$si'>\n";if(!isset($_GET["import"])&&$Gd){print_fieldset("history",'TÃ¶rtÃ©net',$_GET["history"]!="");for($X=end($Gd);$X;$X=prev($Gd)){$x=key($Gd);list($zg,$ii,$wc)=$X;echo'<a href="'.h(ME."sql=&history=$x").'" class="edit" title="'.'Szerkeszt'.'">'.'Szerkeszt'."</a>"." <span class='time' title='".@date('Y-m-d',$ii)."'>".@date("H:i:s",$ii)."</span>"." <code class='jush-$w'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$zg)))),80,"</code>").($wc?" <span class='time'>($wc)</span>":"")."<br>\n";}echo"<input type='submit' name='clear' value='".'TÃ¶rlÃ©s'."'>\n","<a href='".h(ME."sql=&history=all")."' class='edit-all' title='".'Ã–sszes szerkesztÃ©se'."'>".'Ã–sszes szerkesztÃ©se'."</a>\n","</div></fieldset>\n";}echo'</form>
...
as$C=>$m){if(!isset($m["privileges"][$Ni?"update":"insert"])||$b->fieldName($m)==""||$m["generated"])unset($n[$C]);}if($_POST&&!$l&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($Ni?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$v=indexes($a);$Ii=unique_array($_GET["where"],$v);$Bg="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,'A tÃ©tel tÃ¶rÃ¶lve.',$k->delete($a,$Bg,!$Ii));else{$N=array();foreach($n
as$C=>$m){$X=process_input($m);if($X!==false&&$X!==null)$N[idf_escape($C)]=$X;}if($Ni){if(!$N)redirect($A);queries_redirect($A,'A tÃ©tel frissÃ­tve.',$k->update($a,$N,$Bg,!$Ii));if(is_ajax()){page_headers();page_messages($l);exit;}}else{$H=$k->insert($a,$N);$ue=($H?last_id():0);queries_redirect($A,sprintf('%s tÃ©tel beszÃºrva.',($ue?" $ue":"")),$H);}}}$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($Z){$L=array();foreach($n
...
<p>
';if(support("columns")||$a==""){echo'TÃ¡bla nÃ©v: <input name="name" data-maxlength="64" value="',h($J["name"]),'" autocapitalize="off">
';if($a==""&&!$_POST)echo
script("focus(qs('#form')['name']);");echo($Cc?"<select name='Engine'>".optionlist(array(""=>"(".'motor'.")")+$Cc,$J["Engine"])."</select>".on_help("getTarget(event).value",1).script("qsl('select').onchange = helpClose;"):""),' ',($nb&&!preg_match("~sqlite|mssql~",$w)?html_select("Collation",array(""=>"(".'egybevetÃ©s'.")")+$nb,$J["Collation"]):""),' <input type="submit" value="MentÃ©s">
';}echo'
...
<p>
Automatikus nÃ¶velÃ©s: <input type="number" name="Auto_increment" size="6" value="',h($J["Auto_increment"]),'">
',checkbox("defaults",1,($_POST?$_POST["defaults"]:adminer_setting("defaults")),'AlapÃ©rtelmezett Ã©rtÃ©kek',"columnShow(this.checked, 5)","jsonly");$vb=($_POST?$_POST["comments"]:adminer_setting("comments"));echo(support("comment")?checkbox("comments",1,$vb,'MegjegyzÃ©s',"editingCommentsClick(this, true);","jsonly").' '.(preg_match('~\n~',$J["Comment"])?"<textarea name='Comment' rows='2' cols='20'".($vb?"":" class='hidden'").">".h($J["Comment"])."</textarea>":'<input name="Comment" value="'.h($J["Comment"]).'" data-maxlength="'.(min_version(5.5)?2048:60).'"'.($vb?"":" class='hidden'").'>'):''),'<p>
<input type="submit" value="MentÃ©s">
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="Eldob">',confirm(sprintf('Drop %s?',$a));}if(support("partitioning")){$Yf=preg_match('~RANGE|LIST~',$J["partition_by"]);print_fieldset("partition",'ParticionÃ¡lÃ¡s ezzel',$J["partition_by"]);echo'<p>
...
ParticiÃ³k: <input type="number" name="partitions" class="size',($Yf||!$J["partition_by"]?" hidden":""),'" value="',h($J["partitions"]),'">
<table cellspacing="0" id="partition-table"',($Yf?"":" class='hidden'"),'>
<thead><tr><th>ParticiÃ³ neve<th>Ã‰rtÃ©kek</thead>
...
as$C=>$Mc)$c[]=array($Mc["type"],$C,"DROP");if(!$c)redirect(ME."table=".urlencode($a));queries_redirect(ME."table=".urlencode($a),'Az indexek megvÃ¡ltoztak.',alter_indexes($a,$c));}page_header('Indexek',$l,array("table"=>$a),h($a));$n=array_keys(fields($a));if($_POST["add"]){foreach($J["indexes"]as$x=>$u){if($u["columns"][count($u["columns"])]!="")$J["indexes"][$x]["columns"][]="";}$u=end($J["indexes"]);if($u["type"]||array_filter($u["columns"],'strlen'))$J["indexes"][]=array("columns"=>array(1=>""));}if(!$J){foreach($v
as$x=>$u){$v[$x]["name"]=$x;$v[$x]["columns"][]="";}$v[]=array("columns"=>array(1=>""));$J["indexes"]=$v;}echo'
<form action="" method="post">
<div class="scrollable">
<table cellspacing="0" class="nowrap">
<thead><tr>
<th id="label-type">Index tÃ­pusa
<th><input type="submit" class="wayoff">Oszop (mÃ©ret)
<th id="label-name">NÃ©v
<th><noscript>',"<input type='image' class='icon' name='add[0]' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.8.4")."' alt='+' title='".'KÃ¶vetkezÅ‘ hozzÃ¡adÃ¡sa'."'>",'</noscript>
</thead>
';if($qg){echo"<tr><td>PRIMARY<td>";foreach($qg["columns"]as$x=>$d){echo
select_input(" disabled",$n,$d),"<label><input disabled type='checkbox'>".'csÃ¶kkenÅ‘'."</label> ";}echo"<td><td>\n";}$ke=1;foreach($J["indexes"]as$u){if(!$_POST["drop_col"]||$ke!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$ke][type]",array(-1=>"")+$Rd,$u["type"],($ke==count($J["indexes"])?"indexesAddRow.call(this);":1),"label-type"),"<td>";ksort($u["columns"]);$r=1;foreach($u["columns"]as$x=>$d){echo"<span>".select_input(" name='indexes[$ke][columns][$r]' title='".'Oszlop'."'",($n?array_combine($n,$n):$n),$d,"partial(".($r==count($u["columns"])?"indexesAddColumn":"indexesChangeColumn").", '".js_escape($w=="sql"?"":$_GET["indexes"]."_")."')"),($w=="sql"||$w=="mssql"?"<input type='number' name='indexes[$ke][lengths][$r]' class='size' value='".h($u["lengths"][$x])."' title='".'Hossz'."'>":""),(support("descidx")?checkbox("indexes[$ke][descs][$r]",1,$u["descs"][$x],'csÃ¶kkenÅ‘'):"")," </span>";$r++;}echo"<td><input name='indexes[$ke][name]' value='".h($u["name"])."' autocapitalize='off' aria-labelledby='label-name'>\n","<td><input type='image' class='icon' name='drop_col[$ke]' src='".h(preg_replace("~\\?.*~","",ME)."?file=cross.gif&version=4.8.4")."' alt='x' title='".'EltÃ¡volÃ­tÃ¡s'."'>".script("qsl('input').onclick = partial(editingRemoveRow, 'indexes\$1[type]');");}$ke++;}echo'</table>
</div>
<p>
<input type="submit" value="MentÃ©s">
<input type="hidden" name="token" value="',$si,'">
</form>
';}elseif(isset($_GET["database"])){$J=$_POST;if($_POST&&!$l&&!isset($_POST["add_x"])){$C=trim($J["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Az adatbÃ¡zis eldobva.',drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($C),'Az adadtbÃ¡zis Ã¡tnevezve.',rename_database($C,$J["collation"]));}else{$i=explode("\n",str_replace("\r","",$C));$Nh=true;$te="";foreach($i
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$J["collation"]))$Nh=false;$te=$j;}}restart_session();set_session("dbs",null);queries_redirect(ME."db=".urlencode($te),'Az adatbÃ¡zis lÃ©trejÃ¶tt.',$Nh);}}else{if(!$J["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$J["collation"])?" COLLATE $J[collation]":""),substr(ME,0,-1),'Az adatbÃ¡zis mÃ³dosult.');}}page_header(DB!=""?'AdatbÃ¡zis mÃ³dosÃ­tÃ¡sa':'AdatbÃ¡zis lÃ©trehozÃ¡sa',$l,array(),h(DB));$nb=collations();$C=DB;if($_POST)$C=$J["name"];elseif(DB!="")$J["collation"]=db_collation(DB,$nb);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$ud){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\.\*)?~',$ud,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input name="name" id="name" value="'.h($C).'" data-maxlength="64" autocapitalize="off">')."\n".($nb?html_select("collation",array(""=>"(".'egybevetÃ©s'.")")+$nb,$J["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mariadb'=>"supported-character-sets-and-collations/",'mssql'=>"ms187963.aspx",)):""),script("focus(qs('#name'));"),'<input type="submit" value="MentÃ©s">
';if(DB!="")echo"<input type='submit' name='drop' value='".'Eldob'."'>".confirm(sprintf('Drop %s?',DB))."\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME)."?file=plus.gif&version=4.8.4")."' alt='+' title='".'KÃ¶vetkezÅ‘ hozzÃ¡adÃ¡sa'."'>\n";echo'<input type="hidden" name="token" value="',$si,'">
</form>
';}elseif(isset($_GET["scheme"])){$J=$_POST;if($_POST&&!$l){$z=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$z,'SÃ©ma eldobva.');else{$C=trim($J["name"]);$z.=urlencode($C);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($C),$z,'SÃ©ma lÃ©trejÃ¶tt.');elseif($_GET["ns"]!=$C)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($C),$z,'SÃ©ma mÃ³dosult.');else
redirect($z);}}page_header($_GET["ns"]!=""?'SÃ©ma mÃ³dosÃ­tÃ¡sa':'SÃ©ma lÃ©trehozÃ¡sa',$l);if(!$J)$J["name"]=$_GET["ns"];echo'
<form action="" method="post">
<p><input name="name" id="name" value="',h($J["name"]),'" autocapitalize="off">
',script("focus(qs('#name'));"),'<input type="submit" value="MentÃ©s">
';if($_GET["ns"]!="")echo"<input type='submit' name='drop' value='".'Eldob'."'>".confirm(sprintf('Drop %s?',$_GET["ns"]))."\n";echo'<input type="hidden" name="token" value="',$si,'">
</form>
';}elseif(isset($_GET["call"])){$da=($_GET["name"]?$_GET["name"]:$_GET["call"]);page_header('MeghÃ­v'.": ".h($da),$l);$Yg=routine($_GET["call"],(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$Pd=array();$Of=array();foreach($Yg["fields"]as$r=>$m){if(substr($m["inout"],-3)=="OUT")$Of[$r]="@".idf_escape($m["field"])." AS ".idf_escape($m["field"]);if(!$m["inout"]||substr($m["inout"],0,2)=="IN")$Pd[]=$r;}if(!$l&&$_POST){$Ya=array();foreach($Yg["fields"]as$x=>$m){if(in_array($x,$Pd)){$X=process_input($m);if($X===false)$X="''";if(isset($Of[$x]))$f->query("SET @".idf_escape($m["field"])." = $X");}$Ya[]=(isset($Of[$x])?"@".idf_escape($m["field"]):$X);}$G=(isset($_GET["callf"])?"SELECT":"CALL")." ".table($da)."(".implode(", ",$Ya).")";$Hh=microtime(true);$H=$f->multi_query($G);$_a=$f->affected_rows;echo$b->selectQuery($G,$Hh,!$H);if(!$H)echo"<p class='error'>".error()."\n";else{$g=connect();if(is_object($g))$g->select_db(DB);do{$H=$f->store_result();if(is_object($H))select($H,$g);else
echo"<p class='message'>".lang(array('Rutin meghÃ­vva, %d sor Ã©rintett.','Rutin meghÃ­vva, %d sor Ã©rintett.','Rutin meghÃ­vva, %d sor Ã©rintett.'),$_a)." <span class='time'>".@date("H:i:s")."</span>\n";}while($f->next_result());if($Of)select($f->query("SELECT ".implode(", ",$Of)));}}echo'
<form action="" method="post">
...
<input type="submit" value="MeghÃ­v">
<input type="hidden" name="token" value="',$si,'">
</form>
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];$C=$_GET["name"];$J=$_POST;if($_POST&&!$l&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){$Qe=($_POST["drop"]?'Idegen kulcs eldobva.':($C!=""?'Idegen kulcs mÃ³dosult.':'Idegen kulcs lÃ©trejÃ¶tt.'));$A=ME."table=".urlencode($a);if(!$_POST["drop"]){$J["source"]=array_filter($J["source"],'strlen');ksort($J["source"]);$bi=array();foreach($J["source"]as$x=>$X)$bi[$x]=$J["target"][$x];$J["target"]=$bi;}if($w=="sqlite")queries_redirect($A,$Qe,recreate_table($a,$a,array(),array(),array(" $C"=>($_POST["drop"]?"":" ".format_foreign_key($J)))));else{$c="ALTER TABLE ".table($a);$nc="\nDROP ".($w=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($C);if($_POST["drop"])query_redirect($c.$nc,$A,$Qe);else{query_redirect($c.($C!=""?"$nc,":"")."\nADD".format_foreign_key($J),$A,$Qe);$l='A forrÃ¡s Ã©s cÃ©l oszlopoknak azonos tÃ­pusÃºak legyenek, a cÃ©l oszlopok indexeltek legyenek, Ã©s a hivatkozott adatnak lÃ©teznie kell.'."<br>$l";}}}page_header('Idegen kulcs',$l,array("table"=>$a),h($a));if($_POST){ksort($J["source"]);if($_POST["add"])$J["source"][]="";elseif($_POST["change"]||$_POST["change-js"])$J["target"]=array();}elseif($C!=""){$md=foreign_keys($a);$J=$md[$C];$J["source"][]="";}else{$J["table"]=$a;$J["source"]=array("");}echo'
<form action="" method="post">
';$_h=array_keys(fields($a));if($J["db"]!="")$f->select_db($J["db"]);if($J["ns"]!="")set_schema($J["ns"]);$Hg=array_keys(array_filter(table_status('',true),'fk_support'));$bi=array_keys(fields(in_array($J["table"],$Hg)?$J["table"]:reset($Hg)));$wf="this.form['change-js'].value = '1'; this.form.submit();";echo"<p>".'CÃ©l tÃ¡bla'.": ".html_select("table",$Hg,$J["table"],$wf)."\n";if($w=="pgsql")echo'SÃ©ma'.": ".html_select("ns",$b->schemas(),$J["ns"]!=""?$J["ns"]:$_GET["ns"],$wf);elseif($w!="sqlite"){$Wb=array();foreach($b->databases()as$j){if(!information_schema($j))$Wb[]=$j;}echo'DB'.": ".html_select("db",$Wb,$J["db"]!=""?$J["db"]:$_GET["db"],$wf);}echo'<input type="hidden" name="change-js" value="">
<noscript><p><input type="submit" name="change" value="VÃ¡ltoztat"></noscript>
<table cellspacing="0">
<thead><tr><th id="label-source">ForrÃ¡s<th id="label-target">CÃ©l</thead>
...
<p>
tÃ¶rlÃ©skor: ',html_select("on_delete",array(-1=>"")+explode("|",$vf),$J["on_delete"]),' frissÃ­tÃ©skor: ',html_select("on_update",array(-1=>"")+explode("|",$vf),$J["on_update"]),doc_link(array('sql'=>"innodb-foreign-key-constraints.html",'mariadb'=>"foreign-keys/",'pgsql'=>"sql-createtable.html#SQL-CREATETABLE-REFERENCES",'mssql'=>"ms174979.aspx",'oracle'=>"https://docs.oracle.com/cd/B19306_01/server.102/b14200/clauses002.htm#sthref2903",)),'<p>
<input type="submit" value="MentÃ©s">
<noscript><p><input type="submit" name="add" value="Oszlop hozzÃ¡adÃ¡sa"></noscript>
';if($C!=""){echo'<input type="submit" name="drop" value="Eldob">',confirm(sprintf('Drop %s?',$C));}echo'<input type="hidden" name="token" value="',$si,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$J=$_POST;$Lf="VIEW";if($w=="pgsql"&&$a!=""){$O=table_status($a);$Lf=strtoupper($O["Engine"]);}if($_POST&&!$l){$C=trim($J["name"]);$Ga=" AS\n$J[select]";$A=ME."table=".urlencode($C);$Qe='A nÃ©zet mÃ³dosult.';$T=($_POST["materialized"]?"MATERIALIZED VIEW":"VIEW");if(!$_POST["drop"]&&$a==$C&&$w!="sqlite"&&$T=="VIEW"&&$Lf=="VIEW")query_redirect(($w=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ga,$A,$Qe);else{$di=$C."_adminer_".uniqid();drop_create("DROP $Lf ".table($a),"CREATE $T ".table($C).$Ga,"DROP $T ".table($C),"CREATE $T ".table($di).$Ga,"DROP $T ".table($di),($_POST["drop"]?substr(ME,0,-1):$A),'A nÃ©zet eldobva.',$Qe,'A nÃ©zet lÃ©trejÃ¶tt.',$a,$C);}}if(!$_POST&&$a!=""){$J=view($a);$J["name"]=$a;$J["materialized"]=($Lf!="VIEW");if(!$l)$l=error();}page_header(($a!=""?'NÃ©zet mÃ³dosÃ­tÃ¡sa':'NÃ©zet lÃ©trehozÃ¡sa'),$l,array("table"=>$a),h($a));echo'
<form action="" method="post">
<p>NÃ©v: <input name="name" value="',h($J["name"]),'" data-maxlength="64" autocapitalize="off">
',(support("materializedview")?" ".checkbox("materialized",1,$J["materialized"],'Materialized view'):""),'<p>';textarea("select",$J["select"]);echo'<p>
<input type="submit" value="MentÃ©s">
';if($a!=""){echo'<input type="submit" name="drop" value="Eldob">',confirm(sprintf('Drop %s?',$a));}echo'<input type="hidden" name="token" value="',$si,'">
</form>
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$ce=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Jh=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");$J=$_POST;if($_POST&&!$l){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'Az esemÃ©ny eldobva.');elseif(in_array($J["INTERVAL_FIELD"],$ce)&&isset($Jh[$J["STATUS"]])){$dh="\nON SCHEDULE ".($J["INTERVAL_VALUE"]?"EVERY ".q($J["INTERVAL_VALUE"])." $J[INTERVAL_FIELD]".($J["STARTS"]?" STARTS ".q($J["STARTS"]):"").($J["ENDS"]?" ENDS ".q($J["ENDS"]):""):"AT ".q($J["STARTS"]))." ON COMPLETION".($J["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'Az esemÃ©ny mÃ³dosult.':'Az esemÃ©ny lÃ©trejÃ¶tt.'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$dh.($aa!=$J["EVENT_NAME"]?"\nRENAME TO ".idf_escape($J["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($J["EVENT_NAME"]).$dh)."\n".$Jh[$J["STATUS"]]." COMMENT ".q($J["EVENT_COMMENT"]).rtrim(" DO\n$J[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'EsemÃ©ny mÃ³dosÃ­tÃ¡sa'.": ".h($aa):'EsemÃ©ny lÃ©trehozÃ¡sa'),$l);if(!$J&&$aa!=""){$K=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$J=reset($K);}echo'
<form action="" method="post">
<table cellspacing="0" class="layout">
<tr><th>NÃ©v<td><input name="EVENT_NAME" value="',h($J["EVENT_NAME"]),'" data-maxlength="64" autocapitalize="off">
<tr><th title="datetime">Kezd<td><input name="STARTS" value="',h("$J[EXECUTE_AT]$J[STARTS]"),'">
<tr><th title="datetime">VÃ©ge<td><input name="ENDS" value="',h($J["ENDS"]),'">
<tr><th>Minden<td><input type="number" name="INTERVAL_VALUE" value="',h($J["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$ce,$J["INTERVAL_FIELD"]),'<tr><th>Ãllapot<td>',html_select("STATUS",$Jh,$J["STATUS"]),'<tr><th>MegjegyzÃ©s<td><input name="EVENT_COMMENT" value="',h($J["EVENT_COMMENT"]),'" data-maxlength="64">
<tr><th><td>',checkbox("ON_COMPLETION","PRESERVE",$J["ON_COMPLETION"]=="PRESERVE",'BefejezÃ©skor megÅ‘rzi'),'</table>
<p>';textarea("EVENT_DEFINITION",$J["EVENT_DEFINITION"]);echo'<p>
<input type="submit" value="MentÃ©s">
';if($aa!=""){echo'<input type="submit" name="drop" value="Eldob">',confirm(sprintf('Drop %s?',$aa));}echo'<input type="hidden" name="token" value="',$si,'">
</form>
';}elseif(isset($_GET["procedure"])){$da=($_GET["name"]?$_GET["name"]:$_GET["procedure"]);$Yg=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$J=$_POST;$J["fields"]=(array)$J["fields"];if($_POST&&!process_fields($J["fields"])&&!$l){$If=routine($_GET["procedure"],$Yg);$di="$J[name]_adminer_".uniqid();drop_create("DROP $Yg ".routine_id($da,$If),create_routine($Yg,$J),"DROP $Yg ".routine_id($J["name"],$J),create_routine($Yg,array("name"=>$di)+$J),"DROP $Yg ".routine_id($di,$J),substr(ME,0,-1),'A rutin eldobva.','A rutin mÃ³dosult.','A rutin lÃ©trejÃ¶tt.',$da,$J["name"]);}page_header(($da!=""?(isset($_GET["function"])?'FunkciÃ³ mÃ³dosÃ­tÃ¡sa':'EljÃ¡rÃ¡s mÃ³dosÃ­tÃ¡sa').": ".h($da):(isset($_GET["function"])?'FunkciÃ³ lÃ©trehozÃ¡sa':'EljÃ¡rÃ¡s lÃ©trehozÃ¡sa')),$l);if(!$_POST&&$da!=""){$J=routine($_GET["procedure"],$Yg);$J["name"]=$da;}$nb=get_vals("SHOW CHARACTER SET");sort($nb);$Zg=routine_languages();echo'
<form action="" method="post" id="form">
<p>NÃ©v: <input name="name" value="',h($J["name"]),'" data-maxlength="64" autocapitalize="off">
',($Zg?'Nyelv'.": ".html_select("language",$Zg,$J["language"])."\n":""),'<input type="submit" value="MentÃ©s">
<div class="scrollable">
<table cellspacing="0" class="nowrap">
';edit_fields($J["fields"],$nb,$Yg);if(isset($_GET["function"])){echo"<tr><td>".'VisszatÃ©rÃ©si Ã©rtÃ©k';edit_type("returns",$J["returns"],$nb,array(),($w=="pgsql"?array("void","trigger"):array()));}echo'</table>
',script("editFields();"),'</div>
<p>';textarea("definition",$J["definition"]);echo'<p>
<input type="submit" value="MentÃ©s">
';if($da!=""){echo'<input type="submit" name="drop" value="Eldob">',confirm(sprintf('Drop %s?',$da));}echo'<input type="hidden" name="token" value="',$si,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];$J=$_POST;if($_POST&&!$l){$z=substr(ME,0,-1);$C=trim($J["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$z,'Sorozat eldobva.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($C),$z,'Sorozat lÃ©trejÃ¶tt.');elseif($fa!=$C)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($C),$z,'Sorozat mÃ³dosult.');else
redirect($z);}page_header($fa!=""?'Sorozat mÃ³dosÃ­tÃ¡sa'.": ".h($fa):'Sorozat lÃ©trehozÃ¡sa',$l);if(!$J)$J["name"]=$fa;echo'
<form action="" method="post">
<p><input name="name" value="',h($J["name"]),'" autocapitalize="off">
<input type="submit" value="MentÃ©s">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Eldob'."'>".confirm(sprintf('Drop %s?',$fa))."\n";echo'<input type="hidden" name="token" value="',$si,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];$J=$_POST;if($_POST&&!$l){$z=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$z,'TÃ­pus eldobva.');else
query_redirect("CREATE TYPE ".idf_escape(trim($J["name"]))." $J[as]",$z,'TÃ­pus lÃ©trehozva.');}page_header($ga!=""?'TÃ­pus mÃ³dosÃ­tÃ¡sa'.": ".h($ga):'TÃ­pus lÃ©trehozÃ¡sa',$l);if(!$J)$J["as"]="AS ";echo'
<form action="" method="post">
<p>
';if($ga!="")echo"<input type='submit' name='drop' value='".'Eldob'."'>".confirm(sprintf('Drop %s?',$ga))."\n";else{echo"<input name='name' value='".h($J['name'])."' autocapitalize='off'>\n";textarea("as",$J["as"]);echo"<p><input type='submit' value='".'MentÃ©s'."'>\n";}echo'<input type="hidden" name="token" value="',$si,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$Ci=trigger_options();$J=(array)trigger($C,$a)+array("Trigger"=>$a."_bi");if($_POST){if(!$l&&in_array($_POST["Timing"],$Ci["Timing"])&&in_array($_POST["Event"],$Ci["Event"])&&in_array($_POST["Type"],$Ci["Type"])){$uf=" ON ".table($a);$nc="DROP TRIGGER ".idf_escape($C).($w=="pgsql"?$uf:"");$A=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($nc,$A,'A trigger eldobva.');else{if($C!="")queries($nc);queries_redirect($A,($C!=""?'A trigger mÃ³dosult.':'A trigger lÃ©trejÃ¶tt.'),queries(create_trigger($uf,$_POST)));if($C!="")queries(create_trigger($uf,$J+array("Type"=>reset($Ci["Type"]))));}}$J=$_POST;}page_header(($C!=""?'Trigger mÃ³dosÃ­tÃ¡sa'.": ".h($C):'Trigger lÃ©trehozÃ¡sa'),$l,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0" class="layout">
<tr><th>IdÅ‘<td>',html_select("Timing",$Ci["Timing"],$J["Timing"],"triggerChange(/^".preg_quote($a,"/")."_[ba][iud]$/, '".js_escape($a)."', this.form);"),'<tr><th>EsemÃ©ny<td>',html_select("Event",$Ci["Event"],$J["Event"],"this.form['Timing'].onchange();"),(in_array("UPDATE OF",$Ci["Event"])?" <input name='Of' value='".h($J["Of"])."' class='hidden'>":""),'<tr><th>TÃ­pus<td>',html_select("Type",$Ci["Type"],$J["Type"]),'</table>
<p>NÃ©v: <input name="Trigger" value="',h($J["Trigger"]),'" data-maxlength="64" autocapitalize="off">
...
as$lf=>$Vg){if(preg_match('~^(.+)(\(.*\))?$~U',$lf,$B))grant("REVOKE",array_keys($Vg),$B[2]," ON $B[1] FROM $ff");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'A felhasznÃ¡lÃ³ mÃ³dosult.':'A felhasznÃ¡lÃ³ lÃ©trejÃ¶tt.'),!$l);if($Kb)$f->query("DROP USER $ff");}}page_header((isset($_GET["host"])?'FelhasznÃ¡lÃ³'.": ".h("$ha@$_GET[host]"):'FelhasznÃ¡lÃ³ hozzÃ¡adÃ¡sa'),$l,array("privileges"=>array('','PrivilÃ©giumok')));if($_POST){$J=$_POST;$vd=$df;}else{$J=$_GET+array("host"=>$f->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$J["pass"]=$sf;if($sf!="")$J["hashed"]=true;$vd[(DB==""||$vd?"":idf_escape(addcslashes(DB,"%_\\"))).".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0" class="layout">
<tr><th>Szerver<td><input name="host" data-maxlength="60" value="',h($J["host"]),'" autocapitalize="off">
<tr><th>FelhasznÃ¡lÃ³<td><input name="user" data-maxlength="80" value="',h($J["user"]),'" autocapitalize="off">
<tr><th>JelszÃ³<td><input name="pass" id="pass" value="',h($J["pass"]),'" autocomplete="new-password">
';if(!$J["hashed"])echo
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".'PrivilÃ©giumok'.doc_link(array('sql'=>"grant.html#priv_level"));$r=0;foreach($vd
as$lf=>$ud){echo'<th>'.($lf!="*.*"?"<input name='objects[$r]' value='".h($lf)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Szerver',"Databases"=>'AdatbÃ¡zis',"Tables"=>'TÃ¡bla',"Columns"=>'Oszlop',"Procedures"=>'Rutin',)as$Eb=>$fc){foreach((array)$vg[$Eb]as$ug=>$tb){echo"<tr".odd()."><td".($fc?">$fc<td":" colspan='2'").' lang="en" title="'.h($tb).'">'.h($ug);$r=0;foreach($vd
as$lf=>$ud){$C="'grants[$r][".h(strtoupper($ug))."]'";$Y=$ud[strtoupper($ug)];if($Eb=="Server Admin"&&$lf!=(isset($vd["*.*"])?"*.*":".*"))echo"<td>";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".'EngedÃ©lyezÃ©s'."<option value='0'".($Y=="0"?" selected":"").">".'VisszavonÃ¡s'."</select>";else{echo"<td align='center'><label class='block'>","<input type='checkbox' name=$C value='1'".($Y?" checked":"").($ug=="All privileges"?" id='grants-$r-all'>":">".($ug=="Grant option"?"":script("qsl('input').onclick = function () { if (this.checked) formUncheck('grants-$r-all'); };"))),"</label>";}$r++;}}}echo"</table>\n",'<p>
<input type="submit" value="MentÃ©s">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Eldob">',confirm(sprintf('Drop %s?',"$ha@$_GET[host]"));}echo'<input type="hidden" name="token" value="',$si,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")){if($_POST&&!$l){$pe=0;foreach((array)$_POST["kill"]as$X){if(kill_process($X))$pe++;}queries_redirect(ME."processlist=",lang(array('%d folyamat leÃ¡llÃ­tva.','%d folyamat leÃ¡llÃ­tva.','%d folyamat leÃ¡llÃ­tva.'),$pe),$pe||!$_POST["kill"]);}}page_header('Folyamatok',$l);echo'
<form action="" method="post">
<div class="scrollable">
...
as$x=>$X)echo"<td>".(($w=="sql"&&$x=="Info"&&preg_match("~Query|Killed~",$J["Command"])&&$X!="")||($w=="pgsql"&&$x=="current_query"&&$X!="<IDLE>")||($w=="oracle"&&$x=="sql_text"&&$X!="")?"<code class='jush-$w'>".shorten_utf8($X,100,"</code>").' <a href="'.h(ME.($J["db"]!=""?"db=".urlencode($J["db"])."&":"")."sql=".urlencode($X)).'">'.'KlÃ³noz'.'</a>':h($X));echo"\n";}echo'</table>
</div>
<p>
';if(support("kill")){echo($r+1)."/".sprintf('Ã¶sszesen %d',max_connections()),"<p><input type='submit' value='".'LeÃ¡llÃ­t'."'>\n";}echo'<input type="hidden" name="token" value="',$si,'">
</form>
...
as$le=>$W)$z.=where_link($r++,$le,$W);}$X=select_value($X,$z,$m,$hi);$s=h("val[$Ji][".bracket_escape($x)."]");$Y=null;if(isset($_POST["val"][$Ji][bracket_escape($x)]))$_POST["val"][$Ji][bracket_escape($x)];$vc=!is_array($J[$x])&&is_utf8($X)&&$K[$af][$x]==$J[$x]&&!$sd[$x];$gi=preg_match('~text|lob~',$m["type"]??null);echo"<td id='$s'";if(($_GET["modify"]&&$vc)||$Y!==null){$Ad=h($Y!==null?$Y:$J[$x]);echo">".($gi?"<textarea name='$s' cols='30' rows='".(substr_count($J[$x],"\n")+1)."'>$Ad</textarea>":"<input name='$s' value='$Ad' size='$_e[$x]'>");}else{$De=strpos($X,"<i>â€¦</i>");echo" data-text='".($De?2:($gi?1:0))."'".($vc?"":" data-warning='".h('HasznÃ¡ld a szerkesztÃ©s hivatkozÃ¡st ezen Ã©rtÃ©k mÃ³dosÃ­tÃ¡sÃ¡hoz.')."'").">$X</td>";}}}if($Pa)echo"<td>";$b->backwardKeysPrint($Pa,$K[$af]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n","</div>\n";}if(!is_ajax()){if($K||$E){$Kc=true;if($_GET["page"]!="last"){if($y==""||(count($K)<$y&&($K||!$E)))$pd=($E?$E*$y:0)+count($K);elseif($w!="sql"||!$ge){$pd=($ge?false:found_rows($R,$Z));if($pd<max(1e4,2*($E+1)*$y))$pd=reset(slow_query(count_rows($a,$Z,$ge,$wd)));else$Kc=false;}}$Sf=($y!=""&&($pd===false||$pd>$y||$E));if($Sf){echo(($pd===false?count($K)+1:$pd-$E*$y)>$y?'<p><a href="'.h(remove_from_uri("page")."&page=".($E+1)).'" class="loadmore">'.'Load more data'.'</a>'.script("qsl('a').onclick = partial(selectLoadMore, ".(+$y).", '".'Loading'."â€¦');",""):''),"\n";}}echo"<div class='footer'><div>\n";if($K||$E){if($Sf){$Le=($pd===false?$E+(count($K)>=$y?2:1):floor(($pd-1)/$y));echo"<fieldset>";if($w!="simpledb"){echo"<legend><a href='".h(remove_from_uri("page"))."'>".'oldal'."</a></legend>",script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".'oldal'."', '".($E+1)."')); return false; };"),pagination(0,$E).($E>5?" â€¦":"");for($r=max(1,$E-4);$r<min($Le,$E+5);$r++)echo
pagination($r,$E);if($Le>0){echo($E+5<$Le?" â€¦":""),($Kc&&$pd!==false?pagination($Le,$E):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$Le'>".'utolsÃ³'."</a>");}}else{echo"<legend>".'oldal'."</legend>",pagination(0,$E).($E>1?" â€¦":""),($E?pagination($E,$E):""),($Le>$E?pagination($E+1,$E).($Le>$E+1?" â€¦":""):"");}echo"</fieldset>\n";}echo"<fieldset>","<legend>".'Ã–sszes eredmÃ©nyt mutatÃ¡sa'."</legend>";$kc=($Kc?"":"~ ").$pd;echo
checkbox("all",1,0,($pd!==false?($Kc?"":"~ ").lang(array('%d sor','%d sor','%d sor'),$pd):""),"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$kc' : checked); selectCount('selected2', this.checked || !checked ? '$kc' : checked);")."\n","</fieldset>\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>Modify</legend><div>
<input type="submit" value="MentÃ©s"',($_GET["modify"]?'':' title="'.'Ctrl+click on a value to modify it.'.'"'),'>
</div></fieldset>
<fieldset><legend>Selected <span id="selected"></span></legend><div>
<input type="submit" name="edit" value="Szerkeszt">
<input type="submit" name="clone" value="KlÃ³noz">
<input type="submit" name="delete" value="TÃ¶rlÃ©s">',confirm(),'</div></fieldset>
';}$nd=$b->dumpFormat();foreach((array)$_GET["columns"]as$d){if($d["fun"]){unset($nd['sql']);break;}}if($nd){print_fieldset("export",'Export'." <span id='selected2'></span>");$Pf=$b->dumpOutput();echo($Pf?html_select("output",$Pf,$za["output"])." ":""),html_select("format",$nd,$za["format"])," <input type='submit' name='export' value='".'Export'."'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($_c,'strlen'),$e);}echo"</div></div>\n";if($b->selectImportPrint()){echo"<div>","<a href='#import'>".'ImportÃ¡lÃ¡s'."</a>",script("qsl('a').onclick = partial(toggle, 'import');",""),"<span id='import' class='hidden'>: ","<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$za["format"],1);echo" <input type='submit' name='import' value='".'ImportÃ¡lÃ¡s'."'>","</span>","</div>";}echo"<input type='hidden' name='token' value='$si'>\n","</form>\n",(!$wd&&$L?"":script("tableCheck();"));}}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$O=isset($_GET["status"]);page_header($O?'Ãllapot':'VÃ¡ltozÃ³k');$Zi=($O?show_status():show_variables());if(!$Zi)echo"<p class='message'>".'Nincs megjelenÃ­thetÅ‘ eredmÃ©ny.'."\n";else{echo"<table cellspacing='0'>\n";foreach($Zi
...
as$x=>$X)json_row("sum-$x",format_number($X));json_row("");}elseif($_GET["script"]=="kill")$f->query("KILL ".number($_POST["kill"]));else{foreach(count_tables($b->databases())as$j=>$X){json_row("tables-$j",$X);json_row("size-$j",db_size($j));}json_row("");}exit;}else{$Zh=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($Zh&&!$l&&!$_POST["search"]){$H=true;$Qe="";if($w=="sql"&&$_POST["tables"]&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$H=truncate_tables($_POST["tables"]);$Qe='A tÃ¡bla felszabadÃ­tva.';}elseif($_POST["move"]){$H=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Qe='TÃ¡blÃ¡k Ã¡thelyezve.';}elseif($_POST["copy"]){$H=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Qe='TÃ¡blÃ¡k Ã¡tmÃ¡solva.';}elseif($_POST["drop"]){if($_POST["views"])$H=drop_views($_POST["views"]);if($H&&$_POST["tables"])$H=drop_tables($_POST["tables"]);$Qe='TÃ¡blÃ¡k eldobva.';}elseif($w!="sql"){$H=($w=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$Qe='Tables have been optimized.';}elseif(!$_POST["tables"])$Qe='Nincs tÃ¡bla.';elseif($H=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($J=$H->fetch_assoc())$Qe.="<b>".h($J["Table"])."</b>: ".h($J["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$Qe,$H);}page_header(($_GET["ns"]==""?'AdatbÃ¡zis'.": ".h(DB):'SÃ©ma'.": ".h($_GET["ns"])),$l,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".'TÃ¡blÃ¡k Ã©s nÃ©zetek'."</h3>\n";$Yh=tables_list();if(!$Yh)echo"<p class='message'>".'Nincs tÃ¡bla.'."\n";else{echo"<form action='' method='post'>\n";if(support("table")){echo"<fieldset><legend>".'KeresÃ©s a tÃ¡blÃ¡kban'." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'>",script("qsl('input').onkeydown = partialArg(bodyKeydown, 'search');","")," <input type='submit' name='search' value='".'KeresÃ©s'."'>\n";if($b->operator_regexp!==null){echo"<p><label><input type='checkbox' name='regexp' value='1'".(empty($_POST['regexp'])?'':' checked').'>'.'as a regular expression'.'</label>',doc_link(array('sql'=>'regexp.html','pgsql'=>'functions-matching.html#FUNCTIONS-POSIX-REGEXP'))."</p>\n";}echo"</div></fieldset>\n";if($_POST["search"]&&$_POST["query"]!=""){$_GET["where"][0]["op"]=$b->operator_regexp===null||empty($_POST['regexp'])?"LIKE %%":$b->operator_regexp;search_tables();}}echo"<div class='scrollable'>\n","<table cellspacing='0' class='nowrap checkable'>\n",script("mixin(qsl('table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true)});"),'<thead><tr class="wrap">','<td><input id="check-all" type="checkbox" class="jsonly">'.script("qs('#check-all').onclick = partial(formCheck, /^(tables|views)\[/);",""),'<th>'.'TÃ¡bla','<td>'.'Motor'.doc_link(array('sql'=>'storage-engines.html')),'<td>'.'EgybevetÃ©s'.doc_link(array('sql'=>'charset-charsets.html','mariadb'=>'supported-character-sets-and-collations/')),'<td>'.'MÃ©ret'.doc_link(array('sql'=>'show-table-status.html','pgsql'=>'functions-admin.html#FUNCTIONS-ADMIN-DBOBJECT','oracle'=>'REFRN20286')),'<td>'.'Index hossz'.doc_link(array('sql'=>'show-table-status.html','pgsql'=>'functions-admin.html#FUNCTIONS-ADMIN-DBOBJECT')),'<td>'.'Adat szabad'.doc_link(array('sql'=>'show-table-status.html')),'<td>'.'Automatikus nÃ¶velÃ©s'.doc_link(array('sql'=>'example-auto-increment.html','mariadb'=>'auto_increment/')),'<td>'.'Sorok'.doc_link(array('sql'=>'show-table-status.html','pgsql'=>'catalog-pg-class.html#CATALOG-PG-CLASS','oracle'=>'REFRN20286')),(support("comment")?'<td>'.'MegjegyzÃ©s'.doc_link(array('sql'=>'show-table-status.html','pgsql'=>'functions-info.html#FUNCTIONS-INFO-COMMENT-TABLE')):''),"</thead>\n";$S=0;foreach($Yh
as$C=>$T){$cj=($T!==null&&!preg_match('~table|sequence~i',$T));$s=h("Table-".$C);echo'<tr'.odd().'><td>'.checkbox(($cj?"views[]":"tables[]"),$C,in_array($C,$Zh,true),"","","",$s),'<th>'.(support("table")||support("indexes")?"<a href='".h(ME)."table=".urlencode($C)."' title='".'StruktÃºra'."' id='$s'>".h($C).'</a>':h($C));if($cj){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.'NÃ©zet mÃ³dosÃ­tÃ¡sa'.'">'.(preg_match('~materialized~i',$T)?'Materialized view':'NÃ©zet').'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.'Tartalom'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'TÃ¡bla mÃ³dosÃ­tÃ¡sa'),"Index_length"=>array("indexes",'Index mÃ³dosÃ­tÃ¡sa'),"Data_free"=>array("edit",'Ãšj tÃ©tel'),"Auto_increment"=>array("auto_increment=1&create",'TÃ¡bla mÃ³dosÃ­tÃ¡sa'),"Rows"=>array("select",'Tartalom'),)as$x=>$z){$s=" id='$x-".h($C)."'";echo($z?"<td align='right'>".(support("table")||$x=="Rows"||(support("indexes")&&$x!="Data_length")?"<a href='".h(ME."$z[0]=").urlencode($C)."'$s title='$z[1]'>?</a>":"<span$s>?</span>"):"<td id='$x-".h($C)."'>");}$S++;}echo(support("comment")?"<td id='Comment-".h($C)."'>":"");}echo"<tr><td><th>".sprintf('Ã¶sszesen %d',count($Yh)),"<td>".h($w=="sql"?$f->result("SELECT @@default_storage_engine"):""),"<td>".h(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$x)echo"<td align='right' id='sum-$x'>";echo"</table>\n","</div>\n";if(!information_schema(DB)){echo"<div class='footer'><div>\n";$Wi="<input type='submit' value='".'Vacuum'."'> ".on_help("'VACUUM'");$Bf="<input type='submit' name='optimize' value='".'OptimalizÃ¡l'."'> ".on_help($w=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'");echo"<fieldset><legend>".'Selected'." <span id='selected'></span></legend><div>".($w=="sqlite"?$Wi:($w=="pgsql"?$Wi.$Bf:($w=="sql"?"<input type='submit' value='".'ElemzÃ©s'."'> ".on_help("'ANALYZE TABLE'").$Bf."<input type='submit' name='check' value='".'EllenÅ‘rzÃ©s'."'> ".on_help("'CHECK TABLE'")."<input type='submit' name='repair' value='".'JavÃ­t'."'> ".on_help("'REPAIR TABLE'"):"")))."<input type='submit' name='truncate' value='".'FelszabadÃ­t'."'> ".on_help($w=="sqlite"?"'DELETE'":"'TRUNCATE".($w=="pgsql"?"'":" TABLE'")).confirm()."<input type='submit' name='drop' value='".'Eldob'."'>".on_help("'DROP TABLE'").confirm()."\n";$i=(support("scheme")?$b->schemas():$b->databases());if(count($i)!=1&&$w!="sqlite"){$j=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'ÃthelyezÃ©s mÃ¡sik adatbÃ¡zisba'.": ",($i?html_select("target",$i,$j):'<input name="target" value="'.h($j).'" autocapitalize="off">')," <input type='submit' name='move' value='".'Ãthelyez'."'>",(support("copy")?" <input type='submit' name='copy' value='".'MÃ¡solÃ¡s'."'> ".checkbox("overwrite",1,$_POST["overwrite"],'overwrite'):""),"\n";}echo"<input type='hidden' name='all' value=''>";echo
script("qsl('input').onclick = function () { selectCount('selected', formChecked(this, /^(tables|views)\[/));".(support("table")?" selectCount('selected2', formChecked(this, /^tables\[/) || $S);":"")." }"),"<input type='hidden' name='token' value='$si'>\n","</div></fieldset>\n","</div></div>\n";}echo"</form>\n",script("tableCheck();");}$_=[];$_[]="<a href='".h(ME)."create='>".'TÃ¡bla lÃ©trehozÃ¡sa'."</a>";if(support("view"))$_[]="<a href='".h(ME)."view='>".'NÃ©zet lÃ©trehozÃ¡sa'."</a>";echo
generate_linksbar($_);if(support("routine")){echo"<h3 id='routines'>".'Rutinok'."</h3>\n";$ah=routines();if($ah){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'NÃ©v'.'<td>'.'TÃ­pus'.'<td>'.'VisszatÃ©rÃ©si Ã©rtÃ©k'."<td></thead>\n";odd('');foreach($ah
as$J){$C=($J["SPECIFIC_NAME"]==$J["ROUTINE_NAME"]?"":"&name=".urlencode($J["ROUTINE_NAME"]));echo'<tr'.odd().'>','<th><a href="'.h(ME.($J["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($J["SPECIFIC_NAME"]).$C).'">'.h($J["ROUTINE_NAME"]).'</a>','<td>'.h($J["ROUTINE_TYPE"]),'<td>'.h($J["DTD_IDENTIFIER"]),'<td><a href="'.h(ME.($J["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($J["SPECIFIC_NAME"]).$C).'">'.'MÃ³dosÃ­tÃ¡s'."</a>";}echo"</table>\n";}$_=[];if(support('procedure'))$_[]="<a href='".h(ME)."procedure='>".'EljÃ¡rÃ¡s lÃ©trehozÃ¡sa'."</a>";$_[]="<a href='".h(ME)."function='>".'FunkciÃ³ lÃ©trehozÃ¡sa'."</a>";echo
generate_linksbar($_);}if(support("sequence")){echo"<h3 id='sequences'>".'Sorozatok'."</h3>\n";$oh=get_vals("SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = current_schema() ORDER BY sequence_name");if($oh){echo"<table cellspacing='0'>\n","<thead><tr><th>".'NÃ©v'."</thead>\n";odd('');foreach($oh
as$X)echo"<tr".odd()."><th><a href='".h(ME)."sequence=".urlencode($X)."'>".h($X)."</a>\n";echo"</table>\n";}echo
generate_linksbar(["<a href='".h(ME)."sequence='>".'Sorozat lÃ©trehozÃ¡sa'."</a>"]);}if(support("type")){echo"<h3 id='user-types'>".'FelhasznÃ¡lÃ³i tÃ­pus'."</h3>\n";$Ui=types();if($Ui){echo"<table cellspacing='0'>\n","<thead><tr><th>".'NÃ©v'."</thead>\n";odd('');foreach($Ui
as$X)echo"<tr".odd()."><th><a href='".h(ME)."type=".urlencode($X)."'>".h($X)."</a>\n";echo"</table>\n";}echo
generate_linksbar(["<a href='".h(ME)."type='>".'TÃ­pus lÃ©trehozÃ¡sa'."</a>"]);}if(support("event")){echo"<h3 id='events'>".'EsemÃ©ny'."</h3>\n";$K=get_rows("SHOW EVENTS");if($K){echo"<table cellspacing='0'>\n","<thead><tr><th>".'NÃ©v'."<td>".'ÃœtemzÃ©s'."<td>".'Kezd'."<td>".'VÃ©ge'."<td></thead>\n";foreach($K
as$J){echo"<tr>","<th>".h($J["Name"]),"<td>".($J["Execute at"]?'Megadott idÅ‘ben'."<td>".$J["Execute at"]:'Minden'." ".$J["Interval value"]." ".$J["Interval field"]."<td>$J[Starts]"),"<td>$J[Ends]",'<td><a href="'.h(ME).'event='.urlencode($J["Name"]).'">'.'MÃ³dosÃ­tÃ¡s'.'</a>';}echo"</table>\n";$Ic=$f->result("SELECT @@event_scheduler");if($Ic&&$Ic!="ON")echo"<p class='error'><code class='jush-sqlset'>event_scheduler</code>: ".h($Ic)."\n";}echo
generate_linksbar(["<a href='".h(ME)."event='>".'EsemÃ©ny lÃ©trehozÃ¡sa'."</a>"]);}if($Yh)echo
